        ...
        <div className={styles.postContentBody}>
          <div style={{ padding: '1rem', border: '1px solid #e0e3e5', borderRadius: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', backgroundColor: '#f9f9fa' }}>
            ...
            <div style={{ flex: 1, overflow: 'hidden' }}>
              <a href={formatFileUrl(post.attachment.url)} target="_blank" rel="noopener noreferrer" style={{ color: '#191c1e', textDecoration: 'none', fontWeight: 600, display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {post.attachment.name}
              </a>
            </div>
            <a href={formatFileUrl(post.attachment.url)} onClick={handleDownload} style={{ padding: '0.5rem 1rem', border: '1px solid #e0e3e5', borderRadius: '0.25rem', backgroundColor: '#ffffff', color: '#191c1e', cursor: 'pointer', textDecoration: 'none', fontSize: '0.875rem', fontWeight: 500 }}>
              Tải xuống
            </a>
          </div>
        </div>
